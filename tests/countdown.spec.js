...
test.describe("EB Countdown", () => {
  test("can insert an Countdown block", async ({ admin, editor, page }) => {
    ...
    await expect(page.getByLabel('Editor content').getByText('Days')).toBeVisible();
    await expect(page.getByLabel('Editor content').getByText('Hours')).toBeVisible();
    await expect(page.getByLabel('Editor content').getByText('Minutes')).toBeVisible();
    await expect(page.getByLabel('Editor content').getByText('Seconds')).toBeVisible();
    ...
    await expect(page1.getByText("Seconds")).toBeVisible();
  });
});